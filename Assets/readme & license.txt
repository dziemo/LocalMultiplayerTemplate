...
The exported individual prompts are arranged in folders by platform.  They are 128x128 and all use the same naming convention based (mostly) on the Xbox pad.  Controller buttons are named based on their physical location on to that gamepad.

Keyboard prompts are named after their respective keys, with a few decisions made to help keep numpad/main body keys unique.  The kb_comma.png file for example, actually displays the < symbol.  There are also some extras present depending on whether you're treating left/right control/alt/shift as distinct or not.  This folder also contains prompts for common mouse buttons & functions.
